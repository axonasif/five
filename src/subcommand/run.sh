...
"
	...
	function __use_func() {	
		...
		for _input in "${@}"; do {
			...
			if test -e "$_src/${_parsed_input}.sh"; then { # When we have the file in module level.
			...
			} elif test -e "$_multitux_registrydir/${_parsed_input}.sh"; then { # When we have in bashbox std.
				source_fromFile "$_multitux_registrydir/${_parsed_input}";
				...
				} elif test -d "$_multitux_registrydir/$_dir"; then { # Check in bashbox std.
					source_fromDir "$_multitux_registrydir/$_dir";
...
___self="$0";
EOF
	cat "$_multitux_meta" >> "$_target_workfile";
