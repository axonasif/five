...
function gettop() {
    ...
    local TOPFILE="$_multitux_meta_name";
...
if test ! -d "$_arg_path/$_src_dir_name" || test ! -e "$_arg_path/$_multitux_meta_name"; then {
	_top="$(gettop)";
	if test -n "$_top"; then {
		_arg_path="$_top";
		unset _top;
...
readonly _multitux_meta="$_arg_path/$_multitux_meta_name";
...
case "$FUNCNAME" in
	...
	"subcommand::build" | "subcommand::run")
		...
		io::file::check_newline "$_multitux_meta";
		...
		# Load metadata
		source "$_multitux_meta";

		# Check compatibility with bashbox
		if test -v "$_multitux_compat_var_name"; then {
			local MIN MAX VAR_REF;
			VAR_REF="${!_multitux_compat_var_name}";
			...
			if test -n "$MIN"; then {
				...
				log::error "MIN version is missing from $_multitux_compat_var_name in $_multitux_meta_name" 1 || exit;
			} fi
			...
			log::error "$_multitux_compat_var_name metadata is missing in $_multitux_meta_name" 1 || exit;
		} fi
