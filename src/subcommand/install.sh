function subcommand::install() {
	...
	readonly _registry_meta_file="${_multitux_home}/registry.meta" && touch "$_registry_meta_file";
	...
	function box::syncmeta() {
		...
		readonly _registry_lastsync_file="${_multitux_home}/.registry.lastsync" && touch "$_registry_lastsync_file";
		...
		# Internal variables
		local _lock_file="$_multitux_home/.registry.lastdate";
	...
	function box::parsemeta() {
		...
		if string::matches "$_repo_source" '^file://.*'; then { # Local file path
			...
			_box_dir="$_multitux_registrydir/${_repo_user}_${_repo_name}-${_tag_name}";
			...
			_box_dir="$_multitux_registrydir/${_repo_source}-${_tag_name}";
	...
	for _box in "${@}"; do {	
		...
		# } fi
		# }
		# _box_dir="$_multitux_registrydir/${_repo_name}-${_tag_name}";
		...
		if test -e "$_box_dir"; then {
			if [ "$_arg_force" == "off" ] && [ -e "$_box_dir/$_multitux_meta_name" ]; then {
				continue;
			} else {
				rm -rf "$_box_dir";
			} fi
		...
		if test -e "$_box_dir/$_src_dir_name/main.sh" && test ! -v "EXPORT_USEMOL"; then {
			...
			source "$_box_dir/$_multitux_meta_name";
			_built_executable="$_box_dir/target/release/$CODENAME";
			_install_executable="$_multitux_bindir/$CODENAME";
