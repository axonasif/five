...
install-packages ${_pkg_list[@]} \
	&& for _try in sudo wheel; do if useradd -l -u 33333 -G \$_try -md /home/gitpod -s /bin/bash -p gitpod gitpod; then sed -i.bak -e "s/%\$_try\s\+ALL=(ALL\(:ALL\)\?)\s\+ALL/%\$_try ALL=NOPASSWD:ALL/g" /etc/sudoers && break; fi; done \
